...
Purpose:			Set up all panels and items relating to othello screen, start up and build/display main orthello screen, hold controller class to listin to user imputs
					hold panels: [Bottom] [Center -> left panel & board panel] [Center -> right panel & pink panel & Pieces panel -> text panel & counter panel] 
********************************************************************************/
...
public class OthelloViewController extends JFrame implements ActionListener {
	...
	********************************************************************************/
	private void initialize() {
		...
		JButton btnSubmit = createButton("Submit", "Submit", Color.RED, Color.BLACK, ""); /*JButton made for bottom of screen*/
		...
		for (int i = 0 ; i < 9 ; i++){
			...
			if (i == 8) {		
				for (int x = 0 ; x < 9 ; x++) {				
					if(x == 8) {
						JButton button = createButton(charArray[x], charArray[x], Color.BLACK, Color.WHITE, ""); /*make A-H new button get text value from charArray*/
		...
		chckbxNewCheckBox.setActionCommand("Show Valid Moves");
		...
		/*Center panel -> Right panel -> [Pink panel]*/
		JPanel pinkPanel = new JPanel();
		rightPanel.add(pinkPanel);
		pinkPanel.setPreferredSize( new Dimension(430, 420) );
		pinkPanel.setBackground(Color.PINK);
		//*******************************************
		JLabel pinkLabelP1 = new JLabel("<html>Player 1 initialized with 2 pieces.</html>");		
		pinkPanel.add(pinkLabelP1 );
		pinkLabelP1.setPreferredSize( new Dimension(400, 15) );

		JLabel pinkLabelP2 = new JLabel("<html>Player 2 initialized with 2 pieces.</html>");
		pinkPanel.add(pinkLabelP2);
		pinkLabelP2.setPreferredSize( new Dimension(400, 15) );
		//*******************************************
	...
	In parameters:		String text, String ac, Color fgc, Color bgc, String handler
	Out parameters:		return button
	Version:			1.0
	Author:				Asim Jasarevic
	********************************************************************************/
