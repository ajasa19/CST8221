...
public class OthelloViewController extends JFrame implements ActionListener {
	...
	public class controller implements ActionListener  {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			/*if New Connection called try to connect to server and start a read thread*/
				...
				try {
					...
					serverName = networkDialog.getAddress();
					PORT = networkDialog.getPort();

					pinkLabel.append("Negotiating Connection to " + serverName + " on port " + PORT + "\n"); 
					/*new socket being made*/
					server = new Socket(serverName, PORT);

					pinkLabel.append("Connection successful\n"); 
					pinkLabel.append("Welcome to Asim's Othello Server\n"); 
					pinkLabel.append("Use '/help' for commands\n"); 
					...
					/*new read thread being made*/
					read = new OthelloNetworkController();
					read.start();

					/*turn on and off items now that connect is good*/
					connected = true;
					connectionBound(connected);
				} 
				/*catch any issues while trying to connect to server*/
				catch (Exception ex) {
					pinkLabel.append("Could not connect to Server\n"); 
				}
			}

			/*if Disconnect send a /bye  command to server to avoid complexity*/
			else if (action == "Disconnect ") {

				if (checkMessage("/bye") == 0) {
					sendMessage("/bye");
				}
				connected = false;
				connectionBound(connected);
			...
			/*if submit hit run sendMessage*/
			else if(action == "S") {
				String message = textArea.getText();
				if (checkMessage(message) == 0) {
					sendMessage(message);
				}
			}
	...
	/********************************************************************************
	class name:		OthelloNetworkController
	Purpose				thread class recive messages and display also update user list if new user added
	@param				message (message passed)
	@version			1.0
	@author				Asim Jasarevic
	 ********************************************************************************/
	class OthelloNetworkController extends Thread {
		public void run() {
			String message;
				...
				try {
					...
					if(message != null){     
						...
						if (message.charAt(0) == '[') {
							...
							pinkLabel.append(message+ "\n");
						}
					}
				}
				/*catch any issues while parsing message*/
				catch (IOException ex) {
					pinkLabel.append("message failed to be parsed");
				}
			}
		}
	}
	...
	/********************************************************************************
		...
		Purpose				handle some commands user can use form textarea
		@param				message (message passed)
		@version			1.0
		@author				Asim Jasarevic
	 ********************************************************************************/
	...
	/********************************************************************************
		...
		Purpose				sends message to server and also clears text area
		@param				message (message passed to server)
		@version			1.0
		@author				Asim Jasarevic
	 ********************************************************************************/
	...
	/********************************************************************************
		Function name:		connectionBound
		Purpose				This just turns some items in  jpanel on or off depending on if a connection has been set up to prevent errors 
		@param				connected (see if connection established)
		@version			1.0
		@author				Asim Jasarevic
	 ********************************************************************************/
	public void connectionBound(boolean connected) {
		/*if connection established turn off new connection and turn on submit and disconnect*/
		if (connected == true) {
			mntmNewConnection.setEnabled(false);
			mntmDisconnect.setEnabled(true);
			btnSubmit.setEnabled(true);
		} 
		/*if connection not established turn on new connection and turn off submit and disconnect*/
		else {
			mntmNewConnection.setEnabled(true);
